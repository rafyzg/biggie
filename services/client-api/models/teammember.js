...
const getTeammember = async(req, res, next) => {
    try {
        let teammember = await models.teammember.findByPk(req.teammemberId,{ attributes: {exclude: ['password']} });
        res.send(teammember);
    } catch(err) {
        throw Error(err);
    }
...
const getTeammemberByEmail = async(req, res, next) => {
    try {
        let teammember = await models.teammember.findOne({ where : { emailAddress : req.parmas.teamMemberEmail },
